...
P2 now provides versioned starter libraries for 31 controls, 38 KPI/KRIs, six lifecycle decision gates and eleven sources. Stable IDs connect those records to twelve applicability-aware governance control packs, a machine-readable adoption index, six fictional worked Case Files and five Markdown templates for procurement, incident response, board review, red-team evidence and contributions. Every worked case conforms to the P1 Case File schema, identifies its non-production status and source limitations, and carries indexed control, indicator, gate, source and template references.
