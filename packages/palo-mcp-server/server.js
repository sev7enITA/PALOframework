...
export function createPaloMcpServer(runtime = new GovernanceRuntime(), { exposedTools } = {}) {
  ...
  registerTool("palo_submit_evidence", { description: "Prototype: append HMAC-signed evidence to the SQLite hash chain; assumes a trusted caller and is not a production evidence service.", inputSchema: { claim: jsonObject, decision: jsonObject, outcome: z.enum(["executed", "failed"]), payload: jsonObject.optional() } }, guarded(({ claim, decision, outcome, payload }) => runtime.recordEvidence({ claim, decision, outcome, payload })));
