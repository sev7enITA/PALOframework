...
export function createPaloMcpServer(runtime = new GovernanceRuntime(), { exposedTools, requestContext } = {}) {
  ...
  registerTool("palo_verify_action_authority", { description: "Developer preview: normalize and evaluate an Action Claim through local profile data and draft OPA policy; OIDC requests bind the token tenant to Action Claim 1.3.", inputSchema: { claim: jsonObject, approvalId: z.string().optional() } }, guarded(({ claim, approvalId }) => runtime.verifyAction(tenantBoundClaim(claim), approvalId)));
