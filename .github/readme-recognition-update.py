...
marker = "---\n\n## Overview"
if "## Cite PALO" not in text:
    if marker not in text:
        raise SystemExit("README overview marker not found")
    text = text.replace(marker, "---\n\n" + cite + "## Overview", 1)

start = text.find("## Recognition And Public References")
end = text.find("## Quick Start", start)
if start == -1 or end == -1:
    raise SystemExit("Recognition section markers not found")

recognition = '''## Research, Publication And Public Recognition
...
text = text[:start] + recognition + text[end:]
